...
import com.example.ibsys2.backend.database.WorkplacesDB;
        ...
        public static ArrayList<WorkingTime> CalculateWorkingtimesByProductionList(
                                ArrayList<ProductionItem> productionList) {
                                        ...
                                        for (Workplace workplace : workplaces) {
                                            for (WorkplaceProductMerge workplaceProductMerge : workplace.getDurationsforeachProductWorkplace()) {
                                                int quantity = getProductionQuantityByProductId(productionList, workplaceProductMerge.getProductId());
                                                workplace.setDuration(workplace.getDuration() + workplaceProductMerge.getDurationPerUnit() * quantity);
                                                workplace.getProductionTimes().add(new ProductionTimes(workplaceProductMerge.getProductId(),
                                                        quantity, workplaceProductMerge.getDurationPerUnit()));
                                                System.out.println("Arbeitsplatz: " + workplace.getId() + " Produkt: " + workplaceProductMerge.getProductId()
                                                        + " Menge: " + quantity + " Dauer: " + workplaceProductMerge.getDurationPerUnit() * quantity);
                                            }
                                        }
                                        System.out.println("----------------------");
                                        ...
                                        for (Workplace workplace : workplaces) {
                                            System.out.println(
                                                    workplace.getId() + " : " + workplace.getDuration());
                                        }
                                        System.out.println("----------------------");
                                        System.out.println(
                                        ...
                                        for (WaitingListItem waitingListItem : waitingList) {
                                            for (Workplace workplace : workplaces) {
                                                if (waitingListItem.getWorkplaceId() == workplace.getId()) {
                                                   workplace.setDuration (workplace.getDuration() + waitingListItem.getWaitingTime());
                                                   workplace.setWaitingDuration(workplace.getWaitingDuration() + waitingListItem.getWaitingTime());
                                                }
                                            }
                                        }
                                            ...
                                            for (ProductionItem productionItem : productionList) {
                                                ...
                                                for (WorkplaceProductMerge workplaceProductMerge : workplace.getDurationsforeachProductWorkplace()) {
                                                    if (productionItem.getArticle() == workplaceProductMerge.getProductId()
                                                            && workplaceProductMerge.getWorkplaceId() == workplace.getId()) {
                                                        System.out.println("Rüstzeit für Produkt " + workplaceProductMerge.getProductId()
                                                                + " auf Arbeitsplatz " + workplace.getId() + " beträgt "
                                                                + workplaceProductMerge.getSetupTime());
                                                        if (productionItem.getQuantity() != 0) {
                                                            workplace.setDuration(workplace.getDuration() + workplaceProductMerge.getSetupTime());
                                                            workplace.getSetupTimes().add(new SetupTimes(workplaceProductMerge.getProductId(),
                                                                    workplaceProductMerge.getSetupTime(), 1));
                                                        }
                                                        if (productionItem.getQuantity() == 0) {
                                                            workplace.setupTimes.add(new SetupTimes(workplaceProductMerge.getProductId(),
                                                                    workplaceProductMerge.getSetupTime(), 0));
                                                        }
                                                    }
                                                }
                                        ...
                                        for (Workplace workplace : workplaces) {
                                            int copyOverallDuration = workplace.getDuration();
                                            int shifts = 0;
                                            int overtime = 0;
                                            ...
                                            if (workplace.getDuration() >= 7200) {
                                                shifts = 3;
                                                workingTimes.add(new WorkingTime(workplace.getId(), shifts, 0, workplace.getProductionTimes(),
                                                        workplace.getSetupTimes(), workplace.getWaitingDuration(), copyOverallDuration));
                                                continue;
                                
                                            } else if (workplace.getDuration() >= 4800) {
                                                shifts = 2;
                                                workplace.setDuration(workplace.getDuration() - 4800);
                                            } else if (workplace.getDuration() >= 2400) {
                                                shifts = 1;
                                                workplace.setDuration(workplace.getDuration() - 2400);
                                            } else {
                                                // todo are 0 Shifts and only overtimes allowed?
                                                shifts = 0;
                                            }
                                
                                            if (workplace.getDuration() > 800) {
                                                shifts++;
                                                overtime = 0;
                                                workplace.setDuration(workplace.getDuration() - 2400);
                                            }
                                
                                            if (workplace.getDuration() > 0) {
                                                overtime = workplace.getDuration();
                                            }
                                
                                            workingTimes.add(new WorkingTime(workplace.getId(), shifts, overtime / 5, workplace.getProductionTimes(),
                                                    workplace.getSetupTimes(), workplace.getWaitingDuration(), copyOverallDuration));
                                            System.out.println(workplace.getId() + " : " + shifts + " Schichten, " + overtime / 5 + " Überminuten pro Tag");
                                        }
